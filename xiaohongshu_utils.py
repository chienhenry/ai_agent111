...
""",
            ...
            (
                "user",
                """请根据主题"{theme}"创建小红书内容，并使用以下JSON格式返回：
{{
    "titles": ["标题1", "标题2", "标题3", "标题4", "标题5"],
    "content": "正文内容"
}}
确保titles包含5个标题，且返回的是有效的JSON。
""",
            ),
        ]
    )
    ...
    chain = prompt | model
    result_text = chain.invoke({"theme": theme})

    # 解析模型返回的JSON
    try:
        # 查找JSON内容 - 处理可能的格式问题
        text = (
            result_text.content if hasattr(result_text, "content") else str(result_text)
        )
        ...
        # 创建Xiaohongshu对象
        result = Xiaohongshu(
            titles=data.get("titles", ["没有标题"] * 5),
            content=data.get("content", "没有内容"),
        )

        # 确保有5个标题
        if len(result.titles) < 5:
            result.titles.extend(["没有标题"] * (5 - len(result.titles)))
